 ...
 * Created on 10.12.2013.,07.28.51
 * 
 */
class SeoBehavior extends CActiveRecordBehavior {
    ...
    public $categoryRelation = 'category';
    public $categoryAttribute = 'title';
    public $categoryParentRelation = 'parent';
    public $separator = '/';
    public $view = '';
    private $url = '/';

    public function afterSave($event) {
        $model = Seo::model()->findByAttributes(array('model_name'=>get_class($this->getOwner()), 'model_id'=>$this->getOwner()->id));
        if(!$model)
            $model = new Seo ();
        $model->model_name = get_class($this->getOwner());
        $model->model_id = $this->getOwner()->id;
        $model->url = $this->urlize($this->getOwner()->title);
        if(!$model->save()){print_r($model->errors);exit();};
     ...
     * @return boolean
     */
    protected function seemsUtf8($string) {
